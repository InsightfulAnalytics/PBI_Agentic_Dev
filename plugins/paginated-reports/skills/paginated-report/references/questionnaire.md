...
- **The data.** Which source (semantic model, SQL, Analysis Services, Dataverse, other, or Enter Data for a mockup)? Research it rather than only asking: confirm the real table/column/measure names, the grain, and rough volume. For a semantic model, get the workspace and model (and dataset GUID); for SQL, the server/database, auth, and whether it is on-premises. Author and verify any DAX with the `dax-optimisation` skill before it goes in the report.
