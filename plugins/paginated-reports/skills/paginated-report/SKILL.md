...
- **`semantic-models:dax-optimisation`**: author and validate every DAX query a `PBIDATASET` dataset will run, against the live model, before pasting the `EVALUATE` into `<CommandText>`. Do not hand-roll DAX for a paginated report.
