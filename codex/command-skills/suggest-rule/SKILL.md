---
name: suggest-rule
description: Suggest BPA rules based on a description or model analysis. Use when creating new Best Practice Analyzer rules for Tabular Editor, or when the user asks to generate, draft, or propose a BPA rule.
---

# Suggest BPA Rule

<!-- Ported from plugins/tabular-editor/commands/suggest-rule.md (Claude Code slash command) for Codex. -->

Generate a BPA (Best Practice Analyzer) rule based on the description or model the user provided.
...
2. **Load Skill Context**
   - Read the `bpa-rules` skill (SKILL.md and its references) for reference documentation
   - Review its `references/expression-syntax.md` for available properties and patterns
   - Check its `examples/comprehensive-rules.json` for similar existing rules
