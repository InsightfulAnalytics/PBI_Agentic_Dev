...
> [!NOTE]
> This fork sets every theme's font to Consolas, in place of upstream's `'JetBrainsMono Nerd Font', 'JetBrains Mono', Consolas, monospace` stack. Power BI cannot embed fonts, and Consolas is in Power BI's own font list, so the themes keep their monospace look without asking viewers to install JetBrains Mono. Consolas ships with Windows and Microsoft Office. On a Mac without Office the browser substitutes another font and text widths change, so check tight cards and table columns there.
